...
module Validated_for_fast_path = struct
  ...
  let[@zero_alloc opt] validate (wire : int iarray) : t or_null =
    let fold_result =
      Iarray.Local.foldi wire ~init:Fold_state.Start ~f:(fun index acc item ->
        match acc with
        | Invalid -> Invalid
        | Start ->
  ...
  let[@zero_alloc opt] max_common_version (us : t) (peer : t) : int or_null =
...
let get_protocol (t : t) =
  let protocols, versions =
    Iarray.Local.fold
      ...
      ~init:([], [])
      ~f:(fun (protocols, versions) (v : int) ->
